...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal_with_multiple_features(
        self, input_ids, position_ids, attention_mask, past_key_values, labels,
        visual_features):
        visual_features = [vf.to(input_ids.device) for vf in visual_features]
        image_features = self.get_model().mm_projector(image_features)
    ...
    def prepare_inputs_labels_for_multimodal_with_s3d_features(
        self, input_ids, position_ids, attention_mask, past_key_values, labels,
        images, image_sizes=None
    ):
        if images is None or input_ids.shape[1] == 1:
            return input_ids, position_ids, attention_mask, past_key_values, None, labels
        image_features = images.to(input_ids.device)
        #s3d_projector = nn.Linear(832, self.get_model().config.hidden_size)
        #s3d_projector = s3d_projector.to(input_ids.device)
        #image_features = s3d_projector(image_features)
        image_features = self.get_model().mm_projector(image_features)
        # Let's just add dummy tensors if they do not exist,
        # it is a headache to deal with None all the time.
        # But it is not ideal, and if you have a better idea,
