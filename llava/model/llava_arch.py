...
from llava.constants import IGNORE_INDEX, VIDEO_TOKEN_INDEX, DEFAULT_VIDEO_START_TOKEN, DEFAULT_VIDEO_END_TOKEN
...
class SignLlavaForCausalLM(ABC):  # adapted from LlavaMetaForCausalLM(ABC)
    ...
    def prepare_inputs_and_labels(
        ...
        for clip_data in visual_features:
            projected_vf_dict = {}
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            # Step 2. Separate the inputs to [tokens before the visual features] <video> [tokens after the visual features].
            video_token_indices = [-1] + torch.where(cur_input_ids == VIDEO_TOKEN_INDEX)[0].tolist() + [
            ...
            cur_new_labels.append(torch.full((cur_visual_embeddings.shape[0],), IGNORE_INDEX, device=cur_labels.device,
    ...
    def initialize_vision_tokenizer(self, model_args, tokenizer, sign_model_args):
        ...
        if pretrained_ckpt:
            ...
            print("[SignLLaVA]: Loaded the pretrained weights for embed_tokens from {}.".format(
                sign_model_args['projectors']['pretrained_projector_ckpt']))
